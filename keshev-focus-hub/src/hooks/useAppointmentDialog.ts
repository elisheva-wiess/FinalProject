...
export const useAppointmentDialog = (
  preselectedTherapist: AppointmentDialogProps['preselectedTherapist'],
  refreshHours?: number // פרמטר אופציונלי לרענון
) => {
  const [selectedDate, setSelectedDate] = useState<Date | null>(null);
  const [selectedHour, setSelectedHour] = useState<AvailableHour | null>(null);
  const [availableHours, setAvailableHours] = useState<AvailableHour[]>([]);
  ...
  }, [preselectedTherapist, selectedDate, refreshHours]);
